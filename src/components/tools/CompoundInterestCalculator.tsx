  ...
  return (
    <div className="space-y-2">
          ...
          return (
            <div key={d.year} className="flex-1 flex flex-col justify-end items-center gap-0 relative group" style={{ height: '100%' }}>
              ...
              <div className="absolute bottom-full mb-2 hidden group-hover:block z-20 px-3 py-2 rounded-lg text-xs whitespace-nowrap"
                   style={{ background: '#18181b', border: '1px solid rgba(255,255,255,0.1)', boxShadow: '0 8px 24px rgba(0,0,0,0.4)' }}>
                <div className="text-white font-medium">Year {d.year}</div>
                <div style={{ color: '#34D399' }}>Balance: {formatFull(d.balance)}</div>
                <div style={{ color: '#007CF0' }}>Principal: {formatFull(d.principal)}</div>
                <div style={{ color: '#00DFD8' }}>Interest: {formatFull(d.interest)}</div>
              </div>
              {/* Interest bar */}
              <div
      ...
      <div className="flex gap-1">
        {filtered.map((d) => (
          <div key={d.year} className="flex-1 text-center text-[10px] font-mono" style={{ color: '#52525b' }}>
            {d.year}
          </div>
        ))}
      </div>
      ...
      <div className="flex items-center gap-6 justify-center pt-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs" style={{ color: '#95a2b3' }}>Principal</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-3 h-3 rounded-sm" style={{ background: 'linear-gradient(135deg, #007CF0, #00DFD8)' }} />
          <span className="text-xs" style={{ color: '#95a2b3' }}>Interest earned</span>
        </div>
      </div>
    </div>
  );
...
export default function CompoundInterestCalculator() {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center gap-1 p-1 rounded-xl w-fit flex-wrap" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.06)' }}>
        {frequencies.map((f) => (
          <button
            key={f.value}
            ...
            style={{
              background: frequency === f.value ? 'linear-gradient(135deg, #007CF0, #00DFD8)' : 'transparent',
              color: frequency === f.value ? 'white' : '#95a2b3',
              boxShadow: frequency === f.value ? '0 0 20px -4px rgba(0,124,240,0.4)' : 'none',
            }}
          >
            {f.label}
          </button>
        ...
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div
              ...
              style={{ background: 'linear-gradient(135deg, rgba(0,124,240,0.12), rgba(0,223,216,0.06))', border: '1px solid rgba(0,124,240,0.2)' }}
            >
              <p className="text-[11px] font-medium tracking-widest uppercase mb-2" style={{ color: '#95a2b3' }}>
                Final Balance
              </p>
              <div
          ...
          <div
            ...
            style={{ background: 'rgba(0,124,240,0.06)', border: '1px solid rgba(0,124,240,0.12)' }}
          >
            <svg className="w-5 h-5 shrink-0" style={{ color: '#007CF0' }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          >
            <h3 className="text-sm font-medium mb-4" style={{ color: '#95a2b3' }}>Growth Over Time</h3>
            <GrowthChart data={data} />
          </div>

          {/* Year-by-year table (collapsible) */}
          <details className="group">
            <summary className="flex items-center gap-2 cursor-pointer text-sm font-medium transition-colors hover:text-accent-blue" style={{ color: '#95a2b3' }}>
            ...
            <div className="mt-4 overflow-x-auto rounded-xl" style={{ border: '1px solid rgba(255,255,255,0.06)' }}>
              <table className="w-full text-sm">
                <thead>
                  <tr style={{ background: 'rgba(255,255,255,0.03)' }}>
                    <th className="text-left px-4 py-3 font-medium" style={{ color: '#95a2b3' }}>Year</th>
                    <th className="text-right px-4 py-3 font-medium" style={{ color: '#95a2b3' }}>Balance</th>
                    <th className="text-right px-4 py-3 font-medium" style={{ color: '#95a2b3' }}>Principal</th>
                    <th className="text-right px-4 py-3 font-medium" style={{ color: '#95a2b3' }}>Interest</th>
                  </tr>
                </thead>
                <tbody>
                  {data.map((d) => (
                    <tr key={d.year} className="transition-colors hover:bg-white/[0.02]" style={{ borderTop: '1px solid rgba(255,255,255,0.04)' }}>
                      <td className="px-4 py-2.5 font-mono text-white">{d.year}</td>
                      <td className="px-4 py-2.5 text-right font-mono" style={{ color: '#93C5FD' }}>{formatFull(d.balance)}</td>
                      <td className="px-4 py-2.5 text-right font-mono" style={{ color: '#95a2b3' }}>{formatFull(d.principal)}</td>
